...
const itemDefs = {
  wood_sword:    { id:"wood_sword",    type:"weapon",    name:"ดาบไม้",          icon:"🗡",  atk:5,  price:50  },
  iron_sword:    { id:"iron_sword",    type:"weapon",    name:"ดาบเหล็ก",        icon:"⚔",  atk:15, price:150 },
  silver_sword:  { id:"silver_sword",  type:"weapon",    name:"ดาบเงิน",         icon:"🗡",  atk:28, price:350 },
  cursed_blade:  { id:"cursed_blade",  type:"weapon",    name:"ดาบต้องสาป",      icon:"⚡",  atk:40, dropOnly:true },
  leather_armor: { id:"leather_armor", type:"armor",     name:"ชุดหนัง",         icon:"🛡",  hp:30,  def:2,  price:120 },
  chain_mail:    { id:"chain_mail",    type:"armor",     name:"ชุดโซ่",          icon:"🔗",  hp:60,  def:5,  price:300 },
  plate_armor:   { id:"plate_armor",   type:"armor",     name:"ชุดเกราะเต็ม",   icon:"🛡",  hp:100, def:10, dropOnly:true },
  ring:          { id:"ring",          type:"accessory", name:"แหวนพลัง",        icon:"💍",  atk:3,  price:200 },
  amulet:        { id:"amulet",        type:"accessory", name:"จี้มังกร",         icon:"📿",  atk:6,  hp:20,  price:400 },
...
function renderInventory(player) {
  ...
  for (const itemId of player.inventory) {
    if (!countMap[itemId]) { countMap[itemId] = 0; order.push(itemId); }
    countMap[itemId]++;
  }
  ...
  order.forEach(itemId => {
    ...
    const qtyBadge  = qty > 1 ? `<span style="font-size:0.68rem;color:var(--gold);font-weight:700;margin-left:3px;">×${qty}</span>` : "";
    ...
    card.innerHTML = `
      <div class="inv-name">${item.icon} ${item.name}${qtyBadge} ${typeBadge}</div>
...
socket.on("combat_result", (res) => {
  ...
  if (res.type === "craft_ok")    { addLog(`🔨 ตีสำเร็จ! ${res.icon} <span class="log-buy">${res.itemName}</span>`, "log-buy"); if(document.getElementById("bs-modal").classList.contains("open")) renderBlacksmith(lastRecipes); }
  if (res.type === "craft_fail")  addLog(`❌ ตีไม่สำเร็จ: ${res.reason} (${res.recipeName})`);
...
function craftItem(id) { socket.emit("craft_item", id); }

function renderBlacksmith(recipes) {
  const bsDiv = document.getElementById("bs-content");
  if (!bsDiv) return;
  bsDiv.innerHTML = "";
  if (!recipes || !recipes.length) {
    bsDiv.innerHTML = '<div class="inv-empty">ไม่มีสูตรตอนนี้</div>';
    return;
  }
  const grid = document.createElement("div");
  grid.className = "item-grid";
  ...
  recipes.forEach(rec => {
    ...
    // stats of result
    let stats = [];
    if (res.atk)  stats.push(`⚔ +${res.atk}`);
    if (res.hp)   stats.push(`❤ +${res.hp}`);
    if (res.def)  stats.push(`🛡 +${res.def}`);

    // material list with qty check
    const invCounts = {};
    if (playerData) for (const id of playerData.inventory) invCounts[id] = (invCounts[id]||0)+1;

    const matHtml = rec.materials.map(mat => {
      const have = invCounts[mat.id] || 0;
      const ok   = have >= mat.qty;
      const itm  = itemDefs[mat.id];
      const color = ok ? "#2ecc71" : "#e74c3c";
      return `<span style="color:${color};font-size:0.7rem;">${itm?.icon||"?"} ${itm?.name||mat.id} ${have}/${mat.qty}</span>`;
    }).join("  ");

    const goldOk = playerData && playerData.gold >= rec.goldCost;
    const goldColor = goldOk ? "var(--gold)" : "#e74c3c";
    ...
    div.innerHTML = `
      <div class="item-top">
        ...
        <div>
          ...
          <div class="item-type">${{weapon:"อาวุธ",armor:"เกราะ",accessory:"ประดับ"}[res.type]||""}</div>
        </div>
      </div>
      <div class="item-stats" style="font-size:0.72rem;">${stats.join("  ")}</div>
      <div style="margin:4px 0;font-size:0.7rem;color:var(--text-dim);">วัตถุดิบ: ${matHtml}</div>
      <div class="item-price" style="color:${goldColor};">🪙 ${rec.goldCost}</div>
      <button class="btn btn-buy" onclick="craftItem('${rec.id}')">🔨 ตีอาวุธ</button>
    `;
    grid.appendChild(div);
  });
  bsDiv.appendChild(grid);
}

function openBlacksmith() {
  socket.emit("go_place", "blacksmith");
  setBarActive("bar-bs");
  openModal("bs-modal");
}
